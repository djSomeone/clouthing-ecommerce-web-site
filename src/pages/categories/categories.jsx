...
const Categories = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            setLoading(true);
            try {
                const response = await fetch(`${domain}/user/filterProduct`); // Your API endpoint
                if (!response.ok) {
            ...
            try {
                ...
                for (const key in filters) {
                    if (filters[key].length > 0) {
                        queryParams.append(key, filters[key].join(',')); // Example: categories=Wardrobe,Casual Wear
                    }
                }
    ...
    // ... (rest of your component code remains the same, just remove the initial product data)
...
// ... (FilterGroup component remains the same)
const FilterGroup = ({ title, options, selected, onChange, isColorFilter }) => {
    const [isOpen, setIsOpen] = useState(true);
    const [showAll, setShowAll] = useState(false);
    const itemsToShow = showAll ? options : options.slice(0, 5); // Show limited items initially
